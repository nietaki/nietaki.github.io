...
```elixir
defmodule Gibson.Ecto.AtomType do
  @behaviour Ecto.Type
...
```
cannot load `"inactive"` as type Gibson.Ecto.AtomType for field `status` in schema (...)
```
...
that get used the most frequently on a day-to-day basis. Those modules got loaded
...
from the database TODO jacek continue here
